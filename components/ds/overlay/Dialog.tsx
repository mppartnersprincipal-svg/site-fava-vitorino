...
export function Dialog({ open, onClose, title, children, footer, width = 480 }: DialogProps) {
  ...
  // Esc fecha, scroll da página trava e o foco entra no diálogo enquanto aberto.
  useEffect(() => {
    if (!open) return;
  ...
  return (
    ...
    >
      <div ref={panelRef} role="dialog" aria-modal="true" tabIndex={-1} style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '24px 28px 0' }}>
          ...
          {onClose && (
            <button onClick={onClose} aria-label="Fechar" style={{ all: 'unset', cursor: 'pointer', padding: 6, lineHeight: 0 }}>
              <svg width="14" height="14" viewBox="0 0 14 14" aria-hidden="true"><path d="M2 2l10 10M12 2L2 12" stroke="var(--text-muted)" strokeWidth="1.5" /></svg>
            </button>
          )}
        </div>
